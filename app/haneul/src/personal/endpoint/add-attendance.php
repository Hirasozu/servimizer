...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['qr_code'])) {
        ...
        $selectStmt = $conn->prepare("SELECT tbl_user_id, Status, Serial FROM tbl_user WHERE generated_code = :generated_code");
        ...
        try {
            $stmt = $conn->prepare("INSERT INTO tbl_attendance (tbl_user_id, time_in, Status_QR, Serial_QR) VALUES (:tbl_user_id, :time_in, :Status_QR, :Serial_QR)");
            ...
            $stmt1 = $conn->prepare("UPDATE tbl_user SET Status = :Status WHERE generated_code = :generated_code");            
            $stmt1->bindParam(":generated_code", $qrCode, PDO::PARAM_STR);   
            $stmt1->bindParam(":Status", $Status1, PDO::PARAM_STR);
            $stmt1->execute();

            header("Location: https://www.servimizer.com/app/haneul/src/personal/index.php");
        ...
        echo "
            <script>
                ...
                window.location.href = 'https://www.servimizer.com/app/haneul/src/personal/index.php';
            </script>
        ";
    }
}
